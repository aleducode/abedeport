...
// Function to get real post stats (likes and comments count)
function getPostStats($conn, $post_id) {
    if ($conn === null) {
    ...
    try {
        ...
        if ($result) {
            return [
                'like_count' => intval($result['like_count'] ?? 0),
                'comment_count' => intval($result['comment_count'] ?? 0)
            ];
        }
    } catch(PDOException $e) {
        error_log("getPostStats error: " . $e->getMessage());
    }
...
<head>
	...
	<style>
	/* Hero Section with Fixed Background Styles */
	.hero-section {
		position: relative;
		min-height: 620px;
		display: flex;
	...
	<style>
		...
		.post__meta .meta__item i {
			font-size: 0.875rem;
			opacity: 0.8;
		}

		.post__meta .meta__item--views i {
			color: #6c757d;
		}

		.post__meta .meta__item--likes i {
			color: #6c757d;
			transition: all 0.3s ease;
		}

		.post__meta .meta__item--comments i {
			color: #007bff;
		}

		.post__meta .meta__item a {
			display: inline-flex;
			align-items: center;
			gap: 0.25rem;
			color: inherit;
		...
		.post__meta .meta__item a:hover {
			color: #007bff;
			text-decoration: none;
		}

		.post__meta .meta__item--comments a:hover i {
		...
		/* Add some spacing and consistent styling */
		.post__meta {
			display: flex;
			gap: 1rem;
			align-items: center;
...
<body data-template="template-esports">
	...
	<section class="hero-section" id="hero-section">
		<div class="hero-content">
			<div class="container">
				<div class="hero-text">
					<h5 class="hero-subtitle">INFORMACIÓN ACTUALIZADA - Portal Deportivo Abejorral</h5>
					<h1 class="hero-title">
						<span class="hero-title-main">Deportes</span>
						<span class="hero-title-highlight">Abejorral</span>
					</h1>
					<p class="hero-description">Las noticias más importantes del deporte en Abejorral</p>
					<a href="#news-section" class="btn btn-primary btn-icon-right scroll-link">Ver todas las noticias <i class="fas fa-angle-right"></i></a>
				</div>
			</div>
		</div>
	</section>
		...
		<!-- Content
		================================================== -->
		<div class="site-content" id="news-section">
			<div class="container">
							...
							<?php if (!empty($latestNews)): ?>
								<?php foreach ($latestNews as $index => $post): ?>
									...
									<?php 
										...
										// Get real post stats
										$postStats = getPostStats($conn, $post['id_post']);
									?>
										...
										<div class="posts__item posts__item--card posts__item--<?php echo $categories[0]['class']; ?> card">
											...
											<footer class="posts__footer card__footer">
												...
												<ul class="post__meta meta">
													<li class="meta__item meta__item--likes" title="Me gusta">
														<button class="like-btn" data-post-id="<?php echo $post['id_post']; ?>" data-liked="false">
															<i class="far fa-heart"></i>
															<span class="like-count"><?php echo $postStats['like_count']; ?></span>
														</button>
													</li>
													<li class="meta__item meta__item--comments" title="Comentarios">
														<a href="post.php?slug=<?php echo urlencode($postSlug); ?>#comments">
															<i class="fas fa-comment"></i>
															<?php echo $postStats['comment_count']; ?>
														</a>
													</li>
												</ul>
											</footer>
								...
								<div class="post-grid__item col-sm-6">
									<div class="posts__item posts__item--card posts__item--category-1 card">
										...
										<footer class="posts__footer card__footer">
											...
											<ul class="post__meta meta">
												<li class="meta__item meta__item--likes" title="Me gusta">
													<button class="like-btn" data-post-id="0" data-liked="false">
														<i class="far fa-heart"></i>
			...
			<div class="footer-social">
				<div class="container">
					<ul class="footer-social__list list-unstyled">
						...
						<li class="footer-social__item">
							<a href="mailto:abedeport@gmail.com" class="footer-social__link">
								<span class="footer-social__icon">
									<i class="fab fa-google-plus-g"></i>
								</span>
								<div class="footer-social__txt">
									<span class="footer-social__name">Google+</span>
									<span class="footer-social__user">/abedeport</span>
								</div>
							</a>
						</li>
						<li class="footer-social__item">
							<a href="https://www.instagram.com/abede.port" class="footer-social__link">
								<span class="footer-social__icon">
									<i class="fab fa-instagram"></i>
								</span>
								<div class="footer-social__txt">
									<span class="footer-social__name">Instagram</span>
									<span class="footer-social__user">@abede.port</span>
								</div>
							</a>
						</li>
					</ul>
		...
		$(document).ready(function() {
			...
			function initializeLikes() {
				$('.like-btn').each(function() {
					...
					// Get like status for this post
					$.get('api/likes.php', { post_id: postId })
						.done(function(response) {
							if (response.success !== false) {
								updateLikeButton($btn, response.liked, response.like_count);
							}
						})
						.fail(function() {
							// User not logged in - keep as is but allow clicking
						});
				});
			}
			...
			function toggleLike(postId, $btn) {
				...
				.fail(function(xhr) {
					...
					if (xhr.status === 401) {
						...
						console.error('Error:', response?.error || 'Error al procesar el me gusta');
					}
				})
				.always(function() {
